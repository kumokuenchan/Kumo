...
export default function ExportModal({ documents, collectionName, onClose, onSuccess }: ExportModalProps) {
  ...
  const handleExport = () => {
    try {
      ...
      onSuccess(`Exported ${documents.length} documents as ${format.toUpperCase()}`);
      onClose();
    } catch (error) {
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">{documents.length} documents from {collectionName}</p>
            </div>
          </div>
          <button
            onClick={onClose}
