...
interface BranchFileDiffViewerProps {
  filepath: string;
  branchA: string;
  branchB: string;
}
...
const BranchFileDiffViewer: React.FC<BranchFileDiffViewerProps> = ({ filepath, branchA, branchB }) => {
  ...
  useEffect(() => {
    const loadDiff = async () => {
      if (!gitService || !branchA || !branchB || !filepath) {
        setLoading(false);
        return;
      }
      ...
      setLoading(true);
      try {
        const diff = await gitService.getBranchFileDiff(branchA, branchB, filepath);
        setDiffContent(diff);
      } catch (error) {
        console.error('Error loading branch file diff:', error);
        setDiffContent('');
      } finally {
        setLoading(false);
      }
    ...
    loadDiff();
  }, [gitService, branchA, branchB, filepath]);
        ...
        for (let k = 0; k < maxLen; k++) {
          ...
          rows.push(
            <div key={`${i}-${k}`} className="flex transition-colors">
              <div className={`flex-1 flex border-r border-gray-200 dark:border-gray-700 ${removeLine ? 'bg-red-50 dark:bg-red-900/20 hover:bg-red-100/70 dark:hover:bg-red-900/40' : ''}`}>
                {removeLine ? (
                  <>
                    <div className="w-12 flex-shrink-0 text-right pr-2 text-gray-400 dark:text-gray-500 text-[10px] leading-5 select-none border-r border-red-200 dark:border-red-800">
                      {removeLine.oldLineNum}
                    </div>
                    <div className="flex-1 text-red-800 dark:text-red-200 pl-3 pr-2 leading-5">
                      <span className="inline-block w-4 text-center text-red-600 dark:text-red-400 font-bold text-xs">-</span>
                      <span className="ml-1 font-mono text-xs">{renderContent(removeLine.content)}</span>
                    </div>
                  </>
                ) : (
                  <>
    ...
    <div className="h-full flex flex-col border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden bg-white dark:bg-gray-800">
      <div className="bg-gray-50 dark:bg-gray-700/30 px-3 py-2 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
        ...
        <div className="flex items-center gap-2 mt-2 pt-2 border-t border-gray-200 dark:border-gray-700">
          <span className="text-[10px] text-gray-500 dark:text-gray-400 font-mono">{branchA}</span>
          <svg className="w-3 h-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" />
          </svg>
          <span className="text-[10px] text-gray-500 dark:text-gray-400 font-mono">{branchB}</span>
        </div>
      </div>
...
export default BranchFileDiffViewer;
